...
### 第一步：数据摄入与结构化提取（基于 NotebookLM 降 Token 流程）

数据源规范：参见 `skills/financial-data.md`。为极大幅度降低长文本（10-K、电话会议纪要）对主 LLM 的 Token 消耗，本阶段强制采用 `notebooklm-py` 进行数据降维处理。

#### 阶段 1A：构建专属知识库（0 Token 消耗）
调用 `notebooklm-py` 工具，将目标公司的关键长文本资料注入 NotebookLM 笔记本：
- 财报原文 (10-K/10-Q)
- 电话会议纪要 (Earnings Call Transcripts)
- 多源财务数据网页 (Macrotrends, StockAnalysis, 东方财富等)
*(内部调用示例：`notebooklm source add --url {数据源链接} --file {本地PDF}`)*

#### 阶段 1B：高密事实提取（RAG 提问压缩）
调用 `notebooklm-py ask`，向 NotebookLM 提问，精准提取以下结构化数据，作为后续分析的精简上下文输入：
...
#### 阶段 1C：数据交叉验证（必须执行，使用金融严谨性工具）
NotebookLM 提取数据完成后，**必须调用 `tools/financial_rigor.py` 对关键数据进行程序化验证**，杜绝 LLM 幻觉与心算误差。
